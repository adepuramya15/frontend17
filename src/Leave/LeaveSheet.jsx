...
const LeaveSheet = () => {
 ...
 const fetchLeaveSheet =  async () => {
  try{
    ...
    const response = await axios.get("http://52.229.26.9:8085/api/getSheets", {
        method:'GET',
        headers:{
  ...
  const handleSubmit = async (e) => {
    ...
    try{
      ...
      if(isSubmitted && leaveId){
        const response =  await axios.put(`http://52.229.26.9:8085/api/updateSheet/${leaveId}`, leaveData, {
        ...
        setIsSubmitted(true); // Ensure it remains submitted
          ...
          const response = await axios.post(
            "http://52.229.26.9:8085/api/submitSheet",
            leaveData, {
