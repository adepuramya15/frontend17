...
export default function LeaveEmployee() {
  ...
  if (!employeeId) {
    ...
    return;
      ...
      try {
        ...
        console.log(token)
        const response = await axios.get(`http://52.229.26.9:8085/api/leaves/employee/${employeeId}`, {
          headers:{
  ...
  //   try {
  //     await axios.delete(`http://52.229.26.9:8085/api/leave/delete/${id}`);
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      const token = localStorage.getItem('token')
      await axios.delete(`http://52.229.26.9:8085/api/leaves/delete/${deleteRequestId}`, {
        method:'GET',
        headers:{
          'Authorization' : `Bearer ${token}`,
          'Content-Type' : 'application/json'
