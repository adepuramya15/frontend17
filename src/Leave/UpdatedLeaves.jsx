        ...
        const fetchData = async () => {
          try {
            ...
            const [originResponse] = await Promise.all([
                axios.get(`http://52.229.26.9:8085/api/v1/employeeManager/origin/${employeeId}`, {
                    method: 'GET',
                    headers: {
    ...
    useEffect(() => {
        ...
        const fetchRemainingLeaveDays = async (employeeId, leaveType, leaveStartDate, leaveEndDate) => {
            ...
            try {
                const token = localStorage.getItem('token')
                console.log(token)
                const response = await axios.get('http://52.229.26.9:8085/api/leaves/remaining-leaves', {
                    method:'GET',
                    headers:{
                      'Authorization' : `Bearer ${token}`,
                      'Content-Type' : 'application/json'
...
requiredFields.forEach(field => {
    console.log(`${field}: ${formData[field]}`);
        ...
        try {
            ...
            const url = isEditing
                ? `http://52.229.26.9:8085/api/leaves/update/${formData.id}`
                : `http://52.229.26.9:8085/api/leaves/submit`;
